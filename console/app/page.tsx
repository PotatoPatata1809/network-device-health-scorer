...
export default function Page() {
  const { state, demo, discovered, paused, setPaused, startScenario, startDiscover, stopDemo } = useDeviceState();
    ...
    <main className="h-screen flex flex-col">
      <header className="px-6 py-3.5 border-b border-line bg-panel flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3 text-[12px]">
          <span className={paused ? "text-mut" : "text-green"}>
            {paused ? "❚❚ paused" : "● live"} · {state.devices.length} devices</span>
          <UpdatedAt />
        ...
        <div className="flex flex-1 min-h-0">
          <div className="flex-[1.6] overflow-auto px-6 py-5">
            <PriorityList devices={state.devices} selected={selected} onSelect={setSelected}
                          discovered={demo === "discover" ? discovered : null}
                          highlight={demoDev} />
          </div>
